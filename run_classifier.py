...
#                         print("text:",text)
    ...
    def evaluate(args, is_test):
        ...
        instances_num = input_ids.size()[0]
        print("[M]The number of evaluation instances: ", instances_num)
...
#             print('2-confusion',confusion)
            
            for i in range(confusion.size()[0]):
                p = confusion[i,i].item()/confusion[i,:].sum().item()  # 精确率， （label为真，判断为真的总数）/（判断为真的总数）
                r = confusion[i,i].item()/confusion[:,i].sum().item()  
                # recall， （label为真，判断为真的总数）/（标签为真的总数，即 label为真且判断为真+label为真且判断为假）
                f1 = 2*p*r / (p+r)
                if is_test:
